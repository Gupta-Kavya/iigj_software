...
$settings = postcard_read_settings($builderType);
    ...
    if ($_FILES['background_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($info && isset($extensions[$info['mime']])) {
            ...
            if (move_uploaded_file($_FILES['background_image']['tmp_name'], $targetPath)) {
                ...
                file_put_contents(postcard_settings_file($builderType), json_encode($settings, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), LOCK_EX);
                $message = 'Postcard background image saved.';
            } else {
...
$bgUrl = postcard_background_url($settings);
...
#a4Canvas{width:400px;height:600px;position:relative;<?php echo $bgStyle; ?>background-color:#fff;background-position:center center;background-size:cover;background-repeat:no-repeat;border:1px solid #e5e5e5;border-radius:8px;box-shadow:none;transform-origin:top left}
...
<div id="page-wrapper"><div class="container-fluid a4-builder-page">
    <div class="row"><div class="col-lg-12"><h1 class="page-header"><i class="fa fa-file-pdf-o"></i> Postcard Report Builder</h1></div></div>
    ...
    <ul class="nav nav-tabs settings-tabs">
        ...
        <li><a href="a4Settings.php?type=<?php echo $builderType; ?>">A4 Builder</a></li>
    ...
    <div class="builder-shell">
        ...
        <div class="a4-tools-card">
                ...
                <div class="control-grid" style="margin-bottom:10px;">
                    <div style="grid-column:1 / -1;">
                        ...
                        <select id="a4Orientation" class="form-control">
                            <option value="portrait">Portrait - 10cm x 15cm</option>
                            <option value="landscape">Landscape - 15cm x 10cm</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="save-row">
                <button type="button" id="saveA4Settings" class="btn btn-primary btn-wide">Save Postcard Layout</button>
...
var a4Defaults = <?php echo json_encode(postcard_default_settings($builderType), JSON_UNESCAPED_SLASHES); ?>;
...
var baseW = 1000, baseH = 1500, viewW = 400, viewH = 600;
...
function applyA4Orientation(){
    var orientation = a4Settings.orientation === "landscape" ? "landscape" : "portrait";
    if (orientation === "landscape") {
        baseW = 1500; baseH = 1000; viewW = 600; viewH = 400;
    } else {
        baseW = 1000; baseH = 1500; viewW = 400; viewH = 600;
    }
    a4Settings.orientation = orientation;
...
function renderExtraImages(){
    ...
    $.each(a4Settings.additionalImages || [], function(_, img){
        ...
        $("#a4Canvas").append(box);
        box.css({left:sx(img.x || 0),top:sy(img.y || 0),width:sx(img.w || 150),height:sy(img.h || 100),display:(img.display || "block")});
...
$(function(){
    ...
    $("#uploadExtraImage").on("click", function(){
        ...
        $.ajax({url:"report-builder-image-upload.php",method:"POST",data:form,processData:false,contentType:false,dataType:"json"}).done(function(resp){
            ...
            var img = resp.image;
            img.x = 60; img.y = 60; img.w = 150; img.h = 100; img.display = "block";
    ...
    $("#saveA4Settings").on("click", function(){ $.post("save_postcard_settings.php", {report_type:a4BuilderType,settings:JSON.stringify(collectA4Settings())}, function(resp){ alert(resp.message || "Postcard settings saved."); }, "json").fail(function(){ alert("Unable to save Postcard settings."); }); });
    $("#a4BuilderType").on("change", function(){ window.location.href="postcardSettings.php?type="+encodeURIComponent(this.value); });
    $("#resetA4Defaults").on("click", function(){
        AppConfirm.show("Reset preview to default Postcard layout? Click Save Postcard Layout after checking it.", {
            title: "Reset layout",
            confirmText: "Reset",
            danger: true
