...
if ($search !== '') {
    $like = '%' . $search . '%';
    $where[] = '(u.full_name LIKE ? OR u.company_name LIKE ? OR u.branch_location LIKE ? OR u.email LIKE ? OR u.phone LIKE ? OR u.gst_number LIKE ?)';
    $types .= 'ssssss';
...
$reportCountSql = $hasFormUserId
    ...
    : '0 AS report_count';
$stmt = $conn->prepare("SELECT u.id, u.full_name, u.company_name, u.branch_location, u.email, u.phone, u.gst_number, u.email_verified_at, u.role, u.status, u.last_login_at, u.created_at, u.updated_at,
        {$reportCountSql},
...
<form class="sa-filters" method="get"><input class="form-control" name="q" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search name, branch, email, phone, GST"><select class="form-control" name="status"><option value="">All statuses</option><option value="active" <?php echo $status === 'active' ? 'selected' : ''; ?>>Active</option><option value="inactive" <?php echo $status === 'inactive' ? 'selected' : ''; ?>>Inactive</option></select><button class="btn btn-default">Filter</button></form>
<div class="sa-table-wrap"><table class="table table-hover sa-table"><thead><tr><th>User / Branch</th><th>Contact & GST</th><th>Access</th><th>Agreements</th><th>Bookings</th><th>Reports</th><th>Customers</th><th>Payment Totals</th><th>Dates</th><th></th></tr></thead><tbody>
...
<td class="sa-mini"><span><?php echo htmlspecialchars($user['email']); ?></span><span class="sa-muted"><?php echo htmlspecialchars($user['phone'] ?: 'No phone'); ?></span><span class="sa-muted">GST: <?php echo htmlspecialchars($user['gst_number'] ?: 'Not added'); ?></span><span class="sa-muted">Email: <?php echo $user['email_verified_at'] ? 'Verified' : 'Not verified'; ?></span></td>
...
<td class="sa-mini"><strong><?php echo number_format((int) $user['agreement_count']); ?></strong><span class="sa-muted">Delivered: <?php echo number_format((int) $user['delivered_count']); ?></span><span class="sa-muted">Last: <?php echo htmlspecialchars(super_admin_format_date($user['last_agreement_at'], false)); ?></span></td>
<td class="sa-mini"><span>Total: <?php echo number_format((int) $user['booked_count']); ?></span><span>Pending: <?php echo number_format((int) $user['pending_booking_count']); ?></span><span>Generated: <?php echo number_format((int) $user['generated_booking_count']); ?></span><span>Cancelled: <?php echo number_format((int) $user['cancelled_booking_count']); ?></span></td>
<td><strong><?php echo number_format((int) $user['report_count']); ?></strong></td>
<td><strong><?php echo number_format((int) $user['customer_count']); ?></strong></td>
<td class="sa-mini sa-money"><span>Testing: <?php echo number_format((float) $user['testing_total'], 2); ?></span><span>Due: <?php echo number_format((float) $user['due_total'], 2); ?></span><span>Refund: <?php echo number_format((float) $user['refund_total'], 2); ?></span></td>
<td class="sa-mini"><span>Created: <?php echo htmlspecialchars(super_admin_format_date($user['created_at'], false)); ?></span><span>Updated: <?php echo htmlspecialchars(super_admin_format_date($user['updated_at'], false)); ?></span><span>Login: <?php echo htmlspecialchars(super_admin_format_date($user['last_login_at'])); ?></span></td>
<td><a class="sa-btn" href="super-admin-user.php?id=<?php echo (int) $user['id']; ?>">Open</a></td>
</tr><?php endforeach; ?>
<?php if (!$users): ?><tr><td colspan="10" class="text-center text-muted" style="padding:24px">No users found.</td></tr><?php endif; ?>
</tbody></table></div>
