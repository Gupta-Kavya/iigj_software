...
if (!is_array($submitted)) {
    ...
    echo json_encode(['status' => 'error', 'message' => 'Invalid Postcard settings data.']);
    exit;
}

$reportType = atm_report_type($_POST['report_type'] ?? 'S');
$defaults = postcard_default_settings($reportType);
$settings = $defaults;
$orientation = isset($submitted['orientation']) && $submitted['orientation'] === 'landscape' ? 'landscape' : 'portrait';
$dimensions = postcard_page_dimensions($orientation);
...
$maxX = $dimensions['baseWidth'];
$maxY = $dimensions['baseHeight'];
...
$settings['labelWidth'] = atm_clamp(isset($submitted['labelWidth']) ? $submitted['labelWidth'] : $defaults['labelWidth'], 0, 500);
...
foreach ($defaults['fields'] as $key => $fallback) {
    ...
    $settings['fields'][$key] = [
        ...
        'labelWidth' => isset($item['labelWidth']) && $item['labelWidth'] !== '' ? atm_clamp($item['labelWidth'], 0, 500) : null,
        ...
        'h' => atm_clamp(isset($item['h']) ? $item['h'] : $fallback['h'], 16, $maxY),
    ];
}
...
$settings['additionalImages'] = atm_normalize_additional_images($submitted['additionalImages'] ?? [], $maxX, $maxY, 150, 100);

$saved = file_put_contents(postcard_settings_file($reportType), json_encode($settings, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), LOCK_EX);
if ($saved === false) {
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Unable to save Postcard settings.']);
    exit;
}

echo json_encode(['status' => 'success', 'message' => 'Postcard settings saved.']);
?>
